---
title: "BDAT 603 Project II: Spark Architecture"
...
This document contains the original version of the application using Apache Spark, fulfilling the core project requirements. It demonstrates the use of `sparklyr` to connect to a local cluster, copy the simulated dataset into Spark memory, perform Bernoulli sampling (`sdf_sample`), and execute data filtering operations before collecting the aggregated results back into R.

*Note: This chunk is set to `eval=FALSE` to allow the document to knit smoothly. To launch the dashboard, run the chunk manually within RStudio.*

```{r spark-app, eval=FALSE}
library(shiny)
library(tidyverse)
library(lubridate)
library(dbplyr)
...
# -------------------------------
# 3. Bernoulli sample for app use
# -------------------------------
bernoulli_sample <- sales_tbl %>%
  sdf_sample(fraction = 0.1, replacement = FALSE)

# -------------------------------
# 4. Small helper objects for UI
# -------------------------------
  ...
  output$summaryTable1 <- renderTable({
    ...
    filtered_data() %>%
      ...
      summarise(
        ...
        Median_Expenditure = percentile_approx(expenditure, 0.5, 10000L),
